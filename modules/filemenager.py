...
class HydroDB:
    """Class to randle data with linked or non linked tables
    """
    ...
    def quarry(self, table:str, columns:list=None, filter:str=None):
        ...
        @ tuple(filters = ...) -> recives the filters to be used in the quarry
        ...
        if columns:
            ...
            filtering =_search(table_name= table, db_dir_path= self.default_dir_path, data_index='rows')
    ...
    def add(self, tables_names:list , into:tuple, values:tuple):
        ...
        for table_index in range(len(tables_names)):
            ...
            into[table_index].append("data_log")
            ...
            values[table_index].append(str(datetime.now())) # Adds a datalog to erevy table row
    ...
    def delete(self, tables:list, columns:tuple):
        pass
...
#==========================================================================================================================================#
def _get_columns(table_name:str=None, db_dir_path:str=getcwd()):
    return _open_to_read(table_name=table_name, db_dir_path=db_dir_path, data_index="table_columns")


def _get_rows(table_name:str=None, db_dir_path:str=getcwd()):
...
def _open_to_write(table_name: str, db_dir_path: str = getcwd(), data_index: str = None, data_to_write: dict = {}):
    ...
    print(table_name, db_dir_path, data_index, data_to_write)
    sleep(2)

    start_spiner(f"Adding data to {table_name} ...")
    print(data_to_write)
    ...
    try:
        ...
        success_message("Data writed with sucess")
...
def _create_json_struct(table_name: str, columns: list, primary_key:str) -> dict:
    ...
    default_json_struc: dict = {
        ...
        "meta_data": None,
        "table_columns": ["id", "data_log"] + columns,
        "pk" : primary_key,
        "rows": []
    }
...
def _search(table_name:str= None, columns:list= None, db_dir_path:str=getcwd(), data_index:str=None, filter:str=None):
    table_to_quarry = _open_to_read(table_name=table_name, db_dir_path=db_dir_path, data_index=data_index)

    checker:list = []
    start_spiner("Cheking columns")
    try:
        return _match_filter_command(table_to_filter=table_to_quarry, command=filter, columns=columns)
